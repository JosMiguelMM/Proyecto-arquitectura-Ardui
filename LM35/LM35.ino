...
char datom, key,   number_pressed;
short flag, espacio_vacio;
byte second, minute, hour, dayOfWeek, dayOfMonth, month, year;
int contador, datot[8], ide, cantidad, suma;
...
{direccion2=0x00;
  for (i = 0; i < 2; i++)
  {
    i2c_eeprom_write_byte(0x50, direccion2, clave[i]);
    delay(5);
    direccion2++;
  }
}
...
{direccion2=0x00;
  for (i = 0; i < 2; i++)
  {
    datoe[i] = i2c_eeprom_read_byte(0x50, direccion2);
    delay(5);
    direccion2++;
  }
}
...
{ float tempCr;
  ...
  //float tempCt = ((tempC*5000)/1023)/10; 
...
menu:
  ...
  //teclado_dtmf();
  Serial.println(datoe[0]);
  Serial.println(datoe[1]);
...
Miguel:  ide = 0x13;
  ...
  do{   
         ...
         lcd.clear();
         lcd.setCursor(7, 0);
         lcd.print(number_pressed);
         lcd.setCursor(0, 1);
         lcd.print("D PARA SALIR");
         delay(2000);
  }while(number_pressed!='D');
